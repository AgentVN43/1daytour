...
const MealManagement = ({ infoTraveler, setInfoTraveler }) => {
  const [data, setData] = useState([]);
  console.log("🚀 ~ MealManagement ~ data:", data)

  useEffect(() => {
    const dateList = getDatesInRange(infoTraveler.departureDate, infoTraveler.returnDate);
    ...
    setData(initialData);
  }, [infoTraveler.departureDate, infoTraveler.returnDate]);
    ...
    const columns = [
      ...
      },
      {
        title: 'Số phần', dataIndex: 'portionCount', key: 'portionCount', render: (text, _, index) => (
          <InputNumber min={0} value={text} onChange={(value) => handleFieldChange(dayIndex, index, 'portionCount', value)} />
        )
      },
      {
